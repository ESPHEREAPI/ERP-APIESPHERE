...
@Component({
  ...
  styleUrl: './annulation.component.css'
})
export class AnnulationComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.certService.getMotifs('ANNULATION').subscribe({
      next: m  => this.motifs = m,
      error: () => this.errorMsg = 'ANNULATION.ERROR_LOAD_MOTIFS'
    });
    this.loadHistorique();
  }
  ...
  get stateWarning(): string {
    ...
    if (state.includes('annul')) return 'ANNULATION.ALREADY_CANCELLED';
    return '';
  }
  ...
  confirm(): void {
    ...
    this.isLoading = true;
    this.certService.cancelCertificate(this.resolvedReference, this.motifCode).subscribe({
      next: () => {
        this.isLoading = false; this.isConfirming = false;
        this.successMsg = 'ANNULATION.SUCCESS';
      ...
      error: err => {
        this.isLoading = false; this.isConfirming = false;
        this.errorMsg = err?.error?.message ?? err?.message ?? 'ANNULATION.ERROR_API';
      }
    });
  }
