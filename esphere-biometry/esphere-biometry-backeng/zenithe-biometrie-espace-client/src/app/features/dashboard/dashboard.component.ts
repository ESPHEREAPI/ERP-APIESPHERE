...
import { ReferentielService, SouscripteurActifDTO } from '../../core/services/referentiel.service';
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  // Sélecteur souscripteur (DII / Service Santé)
  ...
  goToAdherentDashboard(): void {
    this.router.navigate(['/dashboard-adherent']);
  }

  ngOnInit(): void {

    // Écoute des données et du chargement
    ...
    // ✅ loadDashboardData() DANS le callback — codeSouscripteur garanti avant l'appel HTTP
    // Flag sessionLoaded pour éviter un rechargement à chaque émission du BehaviorSubject
    this.authService.currentUser$
      .pipe(takeUntil(this.destroy$))
      .subscribe((session: any | null) => {
        ...
        if (!this.sessionLoaded) {
          ...
          if (this.isDIIOrServiceSante()) {
            // DII/Service Santé : charger la liste des souscripteurs actifs
            this.referentielService.getSouscripteursActifs().subscribe({
              next: list => { this.souscripteursActifs = list; },
              error: err => console.error('[Dashboard] Souscripteurs actifs:', err)
            });
          } else {
            // Souscripteur connecté : code = lastname (nom entreprise, ex: "CAMTEL")
  ...
  onSouscripteurChange(): void {
    if (this.selectedSouscripteur) {
      // Les requêtes filtrent par champ "souscripteur" (nom entreprise), pas par police
      this.codeSouscripteur = this.selectedSouscripteur.souscripteur;
      this.loadDashboardData();
    }
  }
  ...
  loadDashboardData(): void {
    ...
    // DII / Service Santé : attendre qu'un souscripteur soit sélectionné dans le dropdown
    // Ne jamais utiliser la session DII comme code souscripteur
    if (this.isDIIOrServiceSante()) {
      if (!this.codeSouscripteur) {
        this.error = null; // pas d'erreur, juste attendre la sélection
        return;
      }
    }
  ...
  createConsommationPeriodeChart(): void {
    ...
    this.chartConsommationPeriode = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          { label: 'Montant Total Ticket Modérateur', data: data.map(s => s.montantTotal),    borderColor: '#007bff', backgroundColor: 'rgba(0,123,255,0.1)',  tension: 0.4, fill: true },
          { label: 'Montant Total Prise En Charge',   data: data.map(s => s.montantRembourse), borderColor: '#28a745', backgroundColor: 'rgba(40,167,69,0.1)', tension: 0.4, fill: true }
        ]
      },
      options: {
        ...
        scales: { y: { beginAtZero: true, ticks: { callback: v => this.dashboardService.formatCurrency(typeof v === 'number' ? v : 0) } } }
      }
    });
  }
    ...
    this.chartTypePrestation = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { callbacks: { label: ctx => `${ctx.label}: ${this.dashboardService.formatCurrency(ctx.parsed ?? 0)} (${data[ctx.dataIndex].pourcentageTotal.toFixed(1)}%)` } }
        }
      }
    });
  }
